...
if(isset($_REQUEST['nextlevel']))
    {
       ...
       if($currentfile=="KML/1---21---21.kml")
       {
       $string=str_replace("KML/","",$currentfile);
       $string1=str_replace(".kml","",$string);
       ...
       else if($currentfile=="KML/1---21---1.kml")
       {
        $string=str_replace("KML/","",$currentfile);
       $string1=str_replace(".kml","",$string);
  ...
  if($mainlocation==$sublocation)
  {
    ...
    if($mainlocation==21 && $sublocation==21)
    {
    $str="KML/".$selectedlocation.'---'.$mainlocation.'---'.$resc["sub_location_temp"].".kml";
    }
    ...
    $str="KML/".$res2['country_id']."/".$mainlocation."---".$resc["sub_location_temp"]."/".$selectedlocation.'---'.$mainlocation.'---'.$resc["sub_location_temp"].".kml" ;
   ...
   elseif($mainlocation!=$sublocation)
   {
      ...
      $str="KML/".$res2[$yt]."/".$res2["nxt_mp_level"].'---'.$res2["nxt_mp_level"]."/".$res2['loc_id'].'---'.$res2["nxt_mp_level"].'---'.$res2["nxt_mp_level"].".kml";
 ...
 $toggle="select main_location,sub_location,".$currentlocation." as location,label_toggle from map_level where main_location=".$mainlocation." and country_id= '".$res2['country_id']."'";
  ...
  for($k=0;$k<count($restoggle);$k++)
  {
    ...
    if($restoggle[$k]['location'].'---'.$restoggle[$k]['main_location'].'---'.$restoggle[$k]['sub_location'] == $string1)
        $str1="checked";
   $str .="<label><input type='radio' name='toggleshow'   value='KML/".$res2['country_id']."/".$restoggle[$k]['main_location']."---".$restoggle[$k]['sub_location']."/".$restoggle[$k]['location'].'---'.$restoggle[$k]['main_location'].'---'.$restoggle[$k]['sub_location']."'".$str1." ><span class='label-text'>".$restoggle[$k]['label_toggle']."</span></input> </label>";
  }
    ...
    if($mainlocation==12 && $sublocation==15 && $fileid==676)
   {
    ...
    $query = yii::$app->db->createCommand($selectdata)->queryOne();
     ...
     $sql3="SELECT label FROM location_type WHERE country_id=".$res2['country_id']." and master_name='".$query['master_table']."'";
      ...
      // if($mainlocation==21 && $sublocation==1)
      //  {
      //    echo "Countries";
      //  }
      if($mainlocation<=7)
      {
       echo $res2["location_name"]; 
      } 
      else
      {
...
$sql= "select master_table  from  biweb.Geo_Hrchy_master  where stat !='R' and refid = ".$parent ;

$res=yii::$app->db2->createCommand($sql)->queryOne();

echo json_encode($res['master_table']);
