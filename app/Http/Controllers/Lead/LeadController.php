...
class LeadController extends Controller
{
    public function index()
    {
        $user = auth()->user();
        if($user->type != "Admin") {
            $searchlist = Lead::where(['created_by' => $user->id])->orderBy('updated_at','desc')->get();
        } else {
            $searchlist = Lead::orderBy('updated_at', 'DESC')->get();
    ...
    public function validemail($id)
    {
        ...
        $backendpath = $bepath . "generate_emails";
        ...
        if(isset($genresult['name']) && $genresult['name']=="Error") {
            $validreqid = "Error";

            $model = Lead::find($id);
            $model->emails = $validreqid;
            $model->save();
    ...
    public function validemail2($id)
    {
        ...
        $bepath = config('globals.backend_path');
        $backendpath = $bepath . "validation/results";
        ...
        $response2 = Http::post($backendpath, [
                'validation_id' => $validreqid
        ]);
        ...
        foreach($validemails as $valmail) {
            ...
            if(!isset($valmail['email_address'])) {
                $allvalids = "Error";
            } else {
    ...
    {
        $bepath = config('globals.backend_path');
        $backendpath = $bepath . "news/everything";
